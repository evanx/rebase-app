...
const uniqueIndexKey = (schema, index) => `${schema.key}:unique:${index.key}:h`
const groupIndexKey = (schema, index, data) =>
   `${schema.key}:${index.key}:group:${index.grouper(data).join(':')}:s`
const scoreIndexKey = (schema, index, data) =>
   `${schema.key}:score:${index.key}:z`
...
module.exports = async (data, { client, schema }) => {
   ...
   const res = await rtx(client, multi => {
      ...
      schema.scoreIndexArray.map(index => {
         multi.zadd(scoreIndexKey(schema, index), data.id, index.scorer(data))
      })
   })
}
