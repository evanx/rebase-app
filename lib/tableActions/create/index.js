...
module.exports = ({ redis, logger }, schema) => async data => {
   if (!data.id) {
      ...
      const [idRes] = await rtx(redis, multi => {
         multi.incr(idKey)
      })
      data.id = idRes
   }
   ...
   redis.watch(dataKey)
   const [existsRes, ...uniqueIndexesExistsRes] = await rtx(redis, multi => {
   ...
   const res = await rtx(redis, multi => {
      Object.keys(data).map(key => {
         multi.hset(dataKey, key, schema.stringifyField(key)(data[key]))
      })
      schema.uniqueIndexArray.map(index => {
         multi.hset(
            uniqueIndexKey(schema, index),
